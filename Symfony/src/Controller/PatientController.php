...
class PatientController extends AbstractController
{
     ...
     */
    public function getRendezvous(PatientRepository $patientsRepository , SerializerInterface $SerializerInterface ){
        $Rendezvous=$patientsRepository->findAll();
        $json=$SerializerInterface ->serialize($Rendezvous,'json',['groups'=>'Patient']);
        dump($json);
        die;
    ...
    public function addPatientFront(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $em->persist($patient);
            $em->flush();
            $this->addFlash('success', 'Merci de choisir notre clinique.');
            $this->addFlash('success', 'Veuillez patienter l\'arrivage de l\'ambulance !');

            return $this->redirectToRoute('afficherPatientfront');
        }
        return $this->render("patient/addPatientFront.html.twig", array("formPatient" => $form->createView()));
    }
    /**
