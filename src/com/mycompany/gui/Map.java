...
public class Map extends Form {
    ...
    public void start(Pharmacie stade) {
        if (current != null) {
            current.show();
            return;
        }
        
        Form hi = new Form("Pharmacie Location");
    ...
    StringTokenizer fruitsTokenizer = new StringTokenizer(fruits, delimiter);
     ...
     while (fruitsTokenizer.hasMoreTokens()) {
      ...
      cnt.zoom(moscone, 13);
       ...
       hi.getToolbar().addMaterialCommandToLeftBar("", FontImage.MATERIAL_ARROW_BACK, e-> new HomeForm(theme).show());
